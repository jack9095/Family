package com.maxxipoint.customlayoutmanager.copy;
...
import com.maxxipoint.customlayoutmanager.stack.CardConfig;
import com.maxxipoint.customlayoutmanager.stack.CardLayoutManager;
import com.maxxipoint.customlayoutmanager.stack.OnSwipeListener;

import java.util.List;


public class CardItemTouchHelperCallbackCopy<T> extends ItemTouchHelper.Callback {

    private final RecyclerView.Adapter adapter;
    private List<T> dataList;
    private OnSwipeListener<T> mListener;

    public CardItemTouchHelperCallbackCopy(@NonNull RecyclerView.Adapter adapter, @NonNull List<T> dataList) {
        this.adapter = checkIsNull(adapter);
        this.dataList = checkIsNull(dataList);
    }

    public CardItemTouchHelperCallbackCopy(@NonNull RecyclerView.Adapter adapter, @NonNull List<T> dataList, OnSwipeListener<T> listener) {
        this.adapter = checkIsNull(adapter);
        this.dataList = checkIsNull(dataList);
...
//                    view.setTranslationY((index - Math.abs(ratio)) * itemView.getMeasuredHeight() / CardConfig.DEFAULT_TRANSLATE_Y);
        ...
        } else if (actionState == ItemTouchHelper.ACTION_STATE_IDLE || actionState == ItemTouchHelper.UP) {
            Toast.makeText(recyclerView.getContext(),"点击事件 touch",Toast.LENGTH_SHORT).show();
        }
    }
