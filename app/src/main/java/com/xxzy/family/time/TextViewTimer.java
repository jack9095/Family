...
//    private final Locale mLocale = LanguageManager.getInstance().isLanguageFa() ? Locale.getDefault() : Locale.US;// 波斯语下显示为波斯数字，其他语言统一为阿拉伯数字
    ...
    private void initPaint(Context context, AttributeSet attrs) {
        ...
        //背景颜色
        mTimeTextBg = typedArray.getColor(R.styleable.TextViewTimer_time_background, ContextCompat.getColor(context, R.color.black));
        mBgWidth = typedArray.getDimension(R.styleable.TextViewTimer_background_width, ToolSize.dp2Px(context, 18));
        mBgHeight = typedArray.getDimension(R.styleable.TextViewTimer_background_height, ToolSize.dp2Px(context, 16));
        int colonColor = typedArray.getColor(R.styleable.TextViewTimer_colon_color, ContextCompat.getColor(context, R.color.black));
        int timeColor = typedArray.getColor(R.styleable.TextViewTimer_time_color, ContextCompat.getColor(context, R.color.white));
        float fontSize = typedArray.getDimension(R.styleable.TextViewTimer_time_size, ToolSize.dp2Px(getContext(), 12));
        mRadius = typedArray.getDimension(R.styleable.TextViewTimer_radius, 5);
        mSepWidth = typedArray.getDimension(R.styleable.TextViewTimer_sep_width, ToolSize.dp2Px(context, 10));
        mTimeBgType = typedArray.getInt(R.styleable.TextViewTimer_time_background_type, TimeBgType.ROUND_CIRCLE);
        mBitmapId = typedArray.getResourceId(R.styleable.TextViewTimer_custom_background, 0);
        //设置背景
     ...
     * @param timeStr 时间字符串
     */
    private void drawTextTimeView(Canvas canvas, float x, String timeStr) {
        initFontBaseLine();
